...
The main challenge is how to use hyperedges to represent a collection of CNOT gates that could be implemented using a single ebit. In this first version of the algorithm, we will group CNOTs together only if they share a common control wire and there are no other gates in between their connections to that wire. For every such a collection of CNOT gates, we will create \textit{a single hyperedge}. The hyperedge's vertices will correspond to the controlling wire and each of the different wires the CNOTs target. Algorithm~\ref{code:buildHypVanilla} receives a circuit as input and builds its hypergraph in this way. 
...
If \(x\) is the control wire of the CNOTs in \(h\), we analyse the reallocation of \(x\) from \(i\) to \(j\) as a multiple step process. First, we move all of the other vertices of hyperedge \(h\) from their current block to \(i\). All of those vertices are target vertices, and thus both \(\lambda_c\) and \(\lambda_e\) are decreased equally, as discussed above. Then, we reallocate all of the vertices of \(h\) from \(i\) to \(j\), which has no effects on the cuts of hyperedge \(h\) (which currently has zero cuts) or in the number of ebits (as all qubits are in the same QPU). Finally, we reallocate each of the other (target) vertices of \(h\) back to their respective blocks, increasing both \(\lambda_c\) and \(\lambda_e\) equally, as discussed above. Notice that all target vertices/wires end up where they began, so the only effect on the partition/distribution has been to move the control vertex/wire \(x\) from \(i\) to \(j\). 

Until this point, we have studied how the application of a primitive affects a single hyperedge. However, a vertex may take part in multiple hyperedges. For each one of the hyperedges, we may follow the method above, and arrive to the conclusion that \(\lambda_c = \lambda_e\) is preserved in each case. Therefore, starting from \(\lambda_c = \lambda_e = 0\), applying the whole sequence of primitives one by one, maintains \(\lambda_c = \lambda_e\).
